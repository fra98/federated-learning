...
class Client:
  ...
  def train_accuracy(self, state_dict) -> tuple[float, float]:
    if state_dict is not None:
      self.net.load_state_dict(state_dict)
    criterion = eval(self.model_config["criterion"])()

    return run_accuracy(device=self.device, dataset=self.trainset, batch_size=self.batch_size, net=self.net, criterion=criterion)
